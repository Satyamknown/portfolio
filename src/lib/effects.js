...
export function initHomeEffects(root) {
  ...
  // ---- Sticker trail (desktop only) ----
  // The custom cursor itself lives in lib/cursor.js, initialized globally
  // from main.jsx since it needs to work on every route, not just Home.
  if (finePointer && !reducedMotion) {
    ...
    window.addEventListener('mousemove', onStickerMove, { passive: true });
    cleanups.push(() => window.removeEventListener('mousemove', onStickerMove));
  }
